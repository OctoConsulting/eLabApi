...
public interface EvidenceRepository extends JpaRepository<Evidence, Integer> {
	...
	@Query(value = "select * from elab.evidence e where e.case_id = :caseID and e.evidence_type = :evidenceType", nativeQuery = true)
	...
	@Query(value = "select e.* from elab.evidence e where e.id in :ids",nativeQuery = true)
