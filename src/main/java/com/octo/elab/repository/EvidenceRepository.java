...
public interface EvidenceRepository extends JpaRepository<Evidence, Integer> {
	...
	@Query(value = "select max(id) from elab.evidence order by e.id", nativeQuery = true)
	...
	@Query(value = "select max(_id) from elab.evidence where evidence_type = :evidenceType order by e.id", nativeQuery = true)
